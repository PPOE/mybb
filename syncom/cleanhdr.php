...
$headerdata = imap_rfc822_parse_headers($header, "forum.piratenpartei.at");
...
if (@is_string($headerdata->message_id)) {
	...
	if (!$pos) {
		...
		if (substr($messageid, -1, 1) == ">") {
			$messageidnew = substr($messageid, 0, -1)."@forum.piratenpartei.at>";
	...
	$mail = $header."\nMessage-ID: <".date("Y.m.d.h.i.s.").uniqid()."@forum.piratenpartei.at>\n".$body;
}
