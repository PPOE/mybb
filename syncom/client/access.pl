...
# -- Configuration:
#
my $ACCESS_URI   = 'http://127.0.0.1/syncom/api/access/'; # edit to match the URI used for auth requests
my $DEFAULT_READ = '!*,pirates.at*'; # edit to match the default read: mask
my $DEFAULT_POST = '!*,pirates.at*'; # edit to match the default post: mask
# ---------------------------------------------------------------------------
...
sub access() {
	if (defined($attributes{username})) {
        ...
        my %access_params = (
                ...
                "virtualhost" => "true",
                "newsmaster" => 'news@forum.piratenpartei.at',
                "domain" => "forum.piratenpartei.at",
                "read"     => $DEFAULT_READ,
                "post"     => $DEFAULT_POST
        );
